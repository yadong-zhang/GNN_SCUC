...
%%
cdf = zeros(num_vars, num_samples, nt);
for i = 1:nt
    cdf(:, :, i) = normcdf(X(:, :, i), 0, sqrt(i));
end

std_rv = zeros(num_vars, num_samples, nt);
for i = 1:nt
    std_rv(:, :, i) = icdf('norm', cdf(:, :, i), 0, 1);
end


%%
corr_matrix = readmatrix('../corr_mat/corr_matrix.csv');   % Read correlation matrix
correlated_CDF = zeros(num_samples, nt);
for i = 1:nt
    correlated_CDF(:, i) = mvncdf(std_rv(:, :, i)', -2*ones(1, num_vars), corr_matrix);
end
...
%     correlated_CDF(:, i) = mvncdf(X(:, :, i)', zeros(1, num_vars), cov_matrix);
    correlated_CDF(:, i) = mvncdf(X(:, :, i)', -2*ones(1, num_vars), corr_matrix);
end
...
%%
x1 = unifrnd(0, 1, [100, 1]);
std_rv1 = icdf('norm', x1, 0, 1);

x2 = unifrnd(0, 1, [100, 1]);
std_rv2= icdf('norm', x2, 0, 1);

x3 = unifrnd(0, 1, [100, 1]);
std_rv3 = icdf('norm', x3, 0, 1);

x4 = unifrnd(0, 1, [100, 1]);
std_rv4 = icdf('norm', x4, 0, 1);

x5 = unifrnd(0, 1, [100, 1]);
std_rv5 = icdf('norm', x5, 0, 1);

x6 = unifrnd(0, 1, [100, 1]);
std_rv6 = icdf('norm', x6, 0, 1);


corr_mat = [1, 0.5; 0.5, 1];

std_rv = cat(2, std_rv1, std_rv2);

cdf = mvncdf(std_rv, zeros(1, 2), corr_mat);
...
% 
% subplot(5, 1, 1);
% histogram(x1, FaceAlpha=0.2, EdgeAlpha=0.2);
% subplot(5, 1, 2);
% subplot()
% histogram(x2, FaceAlpha=0.2, EdgeAlpha=0.2);
% hold on;
% histogram(x3, FaceAlpha=0.2, EdgeAlpha=0.2);
% hold on;
% histogram(x4, FaceAlpha=0.2, EdgeAlpha=0.2);
% hold on;
% histogram(x5, FaceAlpha=0.2, EdgeAlpha=0.2);
% 
...
cdf = mvncdf(std_rv, mu, corr_matrix);
