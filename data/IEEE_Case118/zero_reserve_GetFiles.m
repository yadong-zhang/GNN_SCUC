function [mpc, xgd_table, wind_UC] = zero_reserve_GetFiles()
...
mpc.reserves.req = 0*reserve_req;             % Should be consistent with "num_zones"
...
mpc.reserves.cost = 0*reserve_cost;    % Skip winds and loads

% OPTIONAL max reserve quantities for each gen that belongs to at least 1 zone
% (same order as gens, but skipping any gen that does not belong to any zone)
mpc.reserves.qty = 0*reserve_qty;      % Skip winds and loads
